...
**Document ID:** SRS-002  
**Revision:** 1.2 (Final Draft)  
**Date:** 2026-05-23  
**Standard:** ISO/IEC/IEEE 29148:2018 (간소화 적용)  
**Source PRD:** PRD v0.4 — 안심 CBT 역직구 플랫폼 (2026-04-25)  
**Status:** Final Draft — 바이브코딩 MVP 구현 착수 가능  
**원본 SRS:** SRS-001 v1.1 (2026-05-02) 기반, 정상 경로 중심 재작성  
**변경 이력:**
- v1.0: 초안 (원본 SRS에서 정상 경로 추출, 스택 전환)
- v1.1: C-TEC-005/006 제약 추가, LLM Phase 2 배치, AddressFormat 시드 데이터 추가
- v1.2: 종합 검토(난이도·가능성·효율성) 결과 반영. 비용 산정 보강, 구현 가이드 추가, 문서 상태 Final Draft
...
- **LLM 기능은 MVP에서 미적용**한다. Vercel AI SDK + Gemini API 스택은 제약으로 준비하되, 기능 적용은 Phase 2에서 활성화한다.
...
| ASM-04 | 물류 파트너가 Tracking API를 제공함 (미확정 시 Mock API로 개발·테스트 가능) |
...
| # | 리스크 | 영향 | 완화 |
|---|---|---|---|
| R1 | 세금·관세 계산 부정확 | TLC 오류 → 환불·신뢰 하락 | 5개국 수동 검증 + disclaimer 표시 |
| R2 | 통관 룰 변경 업데이트 지연 | 판정 불일치 → 통관 실패 | Admin CRUD로 운영팀 즉시 대응 |
| R3 | Prisma Cold Start on Vercel Serverless | API 응답 500ms~1.5초 추가 지연 | MVP 초기 트래픽에서 체감 영향 미미. 필요 시 Prisma Accelerate 검토 |
| R4 | SQLite ↔ PostgreSQL 동작 차이 | 로컬 통과·배포 실패 | JSON 내부 필터링은 애플리케이션 레벨로 제한. 통합 테스트는 Supabase 스테이징에서 실행 |
| R5 | Open Exchange Rates 무료 티어 USD base only | KRW→현지통화 직접 변환 불가 | KRW→USD→현지통화 2단계 변환. Open Beta부터 Developer 플랜($12/월) 전환 |
| R6 | 물류 파트너 미확정 | F4 개발 지연 | Mock Tracking API로 병렬 개발 |
...
| ID | 시스템 | 용도 | 비용 (MVP) |
|---|---|---|---|
| EXT-01 | Open Exchange Rates API | 환율 조회 | Free (1K req/월, USD base). Open Beta부터 Developer $12/월 |
| EXT-02 | 물류사 Tracking API | 배송 상태 조회 | 물류 파트너 계약에 따라 상이 (보통 무료) |
| EXT-03 | Stripe | 결제 처리 | 건당 2.9% + $0.30 (국제카드 추가 1%) |

> **Phase 2에서 추가:** Google/Loqate 주소 검증 API, 이메일/푸시 알림 서비스, Google Gemini API (LLM)
...
| ID | 요구사항 | 기준 |
|---|---|---|
| REQ-NF-09 | 월 인프라 비용 | ≤ $100 |

**비용 상세 산정 (검토 완료):**

| 서비스 | 플랜 | 월 비용 | 포함 사항 |
|---|---|---|---|
| Vercel | Pro (1인) | $20 | 1TB 대역폭, 10M Edge Requests, $20 사용 크레딧 |
| Supabase | Pro | $25 | 8GB DB, 100K MAU, 100GB 스토리지 |
| Open Exchange Rates | Free → Developer | $0~$12 | 무료 1K req/월. Open Beta부터 Developer $12/월 |
| Sentry | Free | $0 | 5K 이벤트/월 |
| 도메인 | — | ~$1 | 연간 $12 ÷ 12 |
| **합계 (인프라)** | | **$46~$58** | Stripe 거래 수수료 제외 |

> Stripe 거래 수수료(건당 2.9% + $0.30)는 매출 연동 변동비이므로 인프라 비용에서 제외. GA 초기(2,000명)까지 인프라 비용 $100 이내 유지 가능. 상품 이미지는 Supabase Storage 또는 Cloudinary 무료 티어(25GB/월) 사용 권장 (Vercel 대역폭 절약).
...
> 아래 데이터는 Prisma seed 스크립트(`prisma/seed.ts`)로 초기 투입한다. 바이브코딩 시 이 테이블을 그대로 시드 파일에 포함한다.
...
> **참고:** 위 5개국은 PRD v0.4의 "대표 5개국"에 해당하는 예시이며, 실제 대상국이 확정되면 교체한다. phoneRegex는 국가번호 포함 형식이다.
...
└── prisma/
    ...
    └── seed.ts                # 시드 데이터 (AddressFormat 5개국 등)
```
...
난이도가 낮은 기능부터 시작하여 스택에 익숙해진 후 복잡한 기능으로 진행한다.

| 순서 | 작업 | 예상 소요 | 난이도 |
|---|---|---|---|
| 1 | 프로젝트 초기 설정 (Next.js + Prisma + shadcn/ui) + Supabase 연결 + Vercel 배포 관통 테스트 | 1일 | 🟢 |
| 2 | Prisma 스키마 9개 테이블 + AddressFormat 시드 | 0.5일 | 🟢 |
| 3 | F5 정품 배지 (가장 단순한 DB 조회 → UI 표시) | 1일 | 🟢 |
| 4 | F6 건기식 설명 카드 + Admin 입력 | 1.5일 | 🟢 |
| 5 | F7 빠른 재주문 | 1일 | 🟢 |
| 6 | Admin CRUD 4페이지 (ComplianceRules, PricingRules, Products, Supplement) | 3일 | 🟡 |
| 7 | F3 주소 검증 (시드 데이터 Regex 활용) | 1.5일 | 🟡 |
| 8 | F2 통관 확인 (룰 엔진 로직 + 차단 + 대체 제안) | 2일 | 🟡 |
| 9 | F1 총비용 표시 (PricingRules + 환율 API + TLC 산출) | 3일 | 🟡 |
| 10 | Stripe 결제 연동 | 2일 | 🟡 |
| 11 | 인증 (NextAuth.js 또는 Supabase Auth) + Admin RBAC | 1.5일 | 🟡 |
| 12 | F4 배송 추적 (Cron 폴링 + 상태 매핑 + 가이드) | 2.5일 | 🟡 |
| 13 | i18n (영어/한국어) | 1일 | 🟢 |
| 14 | 통합 테스트 + 디버깅 | 3일 | 🟡 |
| 15 | Vercel 최종 배포 + 도메인 | 0.5일 | 🟢 |
| | **합계** | **약 25일 (5주)** | |

### 11.2 Day 1 체크리스트

프로젝트 첫날에 아래를 모두 관통 테스트하여 환경 문제를 조기에 해결한다.

- [ ] `npx create-next-app` + App Router 설정
- [ ] `npx prisma init` + SQLite 로컬 연결 확인
- [ ] shadcn/ui 컴포넌트 1개(Button) 설치 확인
- [ ] Supabase 프로젝트 생성 + DATABASE_URL 환경변수 설정
- [ ] `npx prisma db push` (Supabase에 스키마 반영)
- [ ] Vercel 프로젝트 연결 + Git Push 배포 성공 확인
- [ ] 배포된 URL에서 "Hello World" 페이지 표시 확인
...
| `EXCHANGE_RATE_API_KEY` | Open Exchange Rates App ID | ✅ |
| `STRIPE_SECRET_KEY` | Stripe 서버 측 키 | ✅ |
| `STRIPE_PUBLISHABLE_KEY` | Stripe 클라이언트 측 키 | ✅ |
| `NEXTAUTH_SECRET` | NextAuth.js 시크릿 (또는 Supabase Auth 사용 시 불필요) | ✅ |
| `NEXTAUTH_URL` | NextAuth.js 콜백 URL | ✅ |
| `GOOGLE_GENERATIVE_AI_API_KEY` | Gemini API 키 (Phase 2 준비, MVP에서는 빈 값) | ❌ Phase 2 |
...
| LLM 통합 | 미정의 | CON-10/11로 스택 제약 명시, 기능은 Phase 2 | MVP 복잡도 관리 |
| 시드 데이터 | 미정의 | AddressFormat 5개국 시드 데이터 명시 | 바이브코딩 시 AI 코드 생성 품질 향상 |
| 비용 산정 | AWS $3,000/월 | Vercel+Supabase $46~$58/월 | 97% 절감 |
| 구현 가이드 | 미정의 | 권장 구현 순서 + Day 1 체크리스트 + 환경변수 | 바이브코딩 실행력 강화 |
| 리스크 | 비즈니스 리스크 6개 | 비즈니스 3개 + 기술 3개 | 스택 전환에 따른 기술 리스크 추가 |
...
| 검토 | 날짜 | 결과 | 문서 |
|---|---|---|---|
| 스택 마이그레이션 계획 | 2026-05-23 | SRS v1.1 → v2.0 전환 계획 수립. 기능 커버리지 충분 확인 | SRS_Migration_Plan.md |
| 바이브코딩 타당성 검토 | 2026-05-23 | 방식 A(완전 바이브코딩) 채택. 정상 경로만으로 스코프 축소 | 바이브코딩 타당성검토.md |
| VCE 변경 적용 Plan | 2026-05-23 | CON-10/11, 시드 데이터 추가. MVP 가치 보존 확인 | SRS_v2_0_VCE_Change_Plan.md |
| 종합 적절성 검토 (난이도·가능성·효율성) | 2026-05-23 | 3개 축 모두 "적절, 추가 조정 불필요" | MVP-개발목표-적절성-종합-검토-보고서.md |

---

*End of SRS-002 v1.2 (Final Draft) — Vibe Coding Edition*
